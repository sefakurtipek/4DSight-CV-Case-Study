...
keyPoints_3, descriptors_3 = sift.detectAndCompute(img_3, None)
...
# Marking the keypoint on the image using circles
img = cv.drawKeypoints(img_3_gray, keyPoints_3, img_3_gray, flags=cv.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
cv2.imwrite('Keypoints2_in_img3.png', img)
...
matches = flann.knnMatch(descriptors_1, descriptors_3, k = 2)
...
if len(good)>MIN_MATCH_COUNT:
    ...
    dst_pts = np.float32([ keyPoints_3[m.trainIdx].pt for m in good_without_list ]).reshape(-1,1,2)
